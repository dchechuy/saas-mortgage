...
- **Fixed two real gaps found during the archived-tenant/mirror-drift adversarial pass**: (1) `/quality/*` routes didn't check `Tenant.is_active`, so an archived tenant's own users could still reach them and see a raw skunkBOX error instead of a clean message — added the same `is_active` check `add_user`/`add_agent` already had (`app/routes/quality.py`); (2) `app/services/agent_sync.py`'s stale-mirror deactivation was incorrectly skipped whenever a tenant's visible-Shared-Agent set legitimately dropped to zero (an overly-conservative guard copied from `tenant_sync.py`'s different-risk-profile tenant-list case) — a newly-unshared Agent's local mirror never got deactivated. Both fixed and covered by new regression tests.
- **`app/skunkbox_client.py`**: `_request()`'s retry/failure log lines now include the tenant UUID (previously logged method/path/exception class only, no tenant context) — closes the "tenant UUID in structured API logs" observability gap for this call path.
- **tests/test_cross_system_adversarial.py** (new, 6 tests): archived tenant blocked from AI Quality reads/writes with a clean message, revoked/invalid service credential handled safely across every AI Quality page (no raw 500, no stale-data fallback), Shared Agent mirror deactivation on unshare (the bug above), a stale Component-edit session surviving a mid-session tenant switch fails closed (404, not applied to the wrong tenant), forged `tenant_id`/`external_id`-shaped fields in a request body have no effect on which tenant a mutation targets, and the archived-collections filter never leaks another tenant's rows.
- **Ownership audit** (`docs/TENANT_ISOLATION_AUDIT.md`, extended): added a full "Cross-System Tenant AI Assets — Cophy-side Ownership Audit" section (model classification, every new surface from Phases 5–7, bulk-op/raw-SQL/background-worker sweep, secret-handling verification) and a PRD §16/§17 acceptance-criteria mapping table with evidence for each criterion. Cross-references saas-platform's own new audit doc, including its one real finding (an MCP tool with no tenant filter, fixed on that side this phase — flagged here since it affected this repo's own "Shared Agents are read/use only" guarantee even though no Cophy-side code was at fault).
- **`docs/OPERATIONS_RUNBOOK.md`** (new): observability current-state assessment (what's logged where, the one accepted gap — no local audit-log table for the new service-credential path, since skunkBOX's own trail is already authoritative for those calls — and why), plus runbooks for partial provisioning failure, tenant archival, service-credential rotation, revoked-credential incident response, reconciliation/mirror-drift, unsharing, and a general incident-response checklist.
- **`docs/ROLLOUT_PLAN.md`** (new): the 8-step staged rollout (schema deploy with management APIs disabled → migration rehearsal → service credential + sync → Shared knowledge/Agents for internal testing → Component/quality flags for Cofficiency → one pilot tenant → monitor → expand tenant-by-tenant), each step mapped to the exact flag/credential/command that implements it, plus a rollback/disable strategy table (per-tenant flag reset, global flag disable, unshare, credential revocation, tenant archival — all reversible, none destructive).
- **`docs/ARCHITECTURE.md`**: added an explicit "What 'Shared' means" section — Cofficiency-owned and read/use-only for every other tenant, never joint ownership, never a copy — placed right before the existing skunkBOX-boundary section so it's read first.
- Confirmed via the migration rehearsal: 0 knowledge collections and 0 Agents are currently marked Shared anywhere — marking one Shared remains a deliberate per-resource Cofficiency admin action, never automated, per the PRD.
- Full suite: 123 tests pass (117 pre-existing + 6 new Phase 8 adversarial tests). saas-platform: 123 passed, 2 pre-existing failures unrelated to tenant isolation (tracked in that repo's own changelog, not fixed here as out of scope).
- No unresolved cross-tenant access path remains open as a **blocking** item for pilot enablement — the one genuine cross-tenant data-disclosure path found (skunkBOX's MCP tool surface) is fixed and tested; the remaining lower-severity findings (documented in both repos' audit docs) are explicitly tracked, not silently deferred.
