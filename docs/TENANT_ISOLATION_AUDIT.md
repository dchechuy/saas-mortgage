...
| §17.9 | A tenant Agent can combine tenant-private and Shared collections | ✅ | System Config → AI Agents lists eligible tenant-owned and Shared Cofficiency collections and replaces associations through skunkBOX's authoritative API. Cross-tenant private IDs are rejected upstream. `tests/test_tenant_completion_phase5.py::test_customer_creates_agent_with_own_and_shared_collections`, `test_agent_edit_rejects_cross_tenant_collection_and_reports_partial_success`. |
